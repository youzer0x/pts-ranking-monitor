...
1. **[開示]＝当該 SESSION 日 15:30 以降の TDnet 開示**（`row.disclosures` に格納済み）。東証通常取引の引けは **15:30**。15:30 以降の開示こそが PTS ナイト（17:00〜翌6:00）の材料になり得る（15:30 より前は日中に織り込み済み）。決算・上方/下方修正・TOB・新株予約権(MSワラント等)・優待・子会社化・大型受注などを**具体的に**記す。
